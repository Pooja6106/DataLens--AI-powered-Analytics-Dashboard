...
class KPIEngine:
    ...
    def _detect_columns(self):
        ...
        self.date_col = find([
            ...
            "timestamp","datetime","day","created",
            "recorded","joining","hire_date",
            "production_date","shift_date","log_date"
        ])

        self.revenue_col = find([
            "revenue","sales","amount","total","price",
            "income","gross","cost","value",
            "production_cost","salary","wage","budget",
            "expense","spend","profit","stock_value",
            "output_value","billing","earning",
            "na_sales","eu_sales","global_sales",
            "amount_of","approved","exemption","claims",
            "tax","production_qty","output","qty",
            "order_qty","stock_qty"
        ])

        self.qty_col = find([
            "quantity","qty","units","count","volume",
            "output","produced","manufactured","pieces",
            "parts","items","throughput","yield",
            "total_units","completed","headcount",
            "stock","inventory","attendance","inspected",
            "production_qty","order_qty","stock_qty"
        ])

        self.order_col = find([
            "order","transaction","invoice","batch","job",
            "batch_id","job_id","work_order","serial",
            "lot","ticket","ref","employee_id","emp_id",
            "sku","item_code","po_number","order_id",
            "taxpayer","claim_id","record_id","po_number",
            "machine_id"
        ])

        self.customer_col = find([
            "customer","client","user","buyer","operator",
            "worker","employee","staff","technician",
            "supplier","vendor","machine","equipment",
            "asset","inspector","taxpayer_name",
            "company","organization","entity","employee_id"
        ])

        self.category_col = find([
            "category","type","segment","department",
            "group","genre","shift","process","operation",
            "stage","designation","role","gender","grade",
            "level","defect_type","fault_type",
            "expense_type","item_type","industry","sector",
            "class","classification","nature",
            "inspection_status","attendance_status"
        ])

        self.region_col = find([
            "region","country","city","location","area",
            "state","plant","factory","facility","site",
            "floor","zone","line","station","machine",
            "warehouse","bin","rack","branch","division",
            "territory","cluster","platform","district",
            "province","municipality","town","zone_code",
            "machine_id","inspector"
        ])

        self.product_col = find([
            "product","item","name","sku","description",
            "title","part","component","material","model",
            "variant","code","part_name","item_name",
            "product_name","material_name","employee_name",
            "machine_name","supplier_name","game",
            "town_code","region_code","project","scheme",
            "program","item_id","po_number","employee_id"
        ])

        self.defect_col = find([
            "defect","reject","scrap","waste","error",
            "fault","failure","rework","return","ncr",
            "complaint","absent","leave","downtime",
            "idle","loss","shortage","rejected_qty",
            "downtime_min","defects"
        ])
        ...
        self.expense_col = find([
            ...
            "outflow","debit","payment","budget_used",
            "actual_cost"
        ])

        # ── If no revenue found, use qty ──
    ...
    # COMPUTE MAIN KPIs
    # ─────────────────────────────────────────
    def compute(self, date_from=None,
                date_to=None, dept=None):
        df = self.df.copy()
        ...
        kpis["total_orders"]    = (
            len(df[self.order_col].dropna())
            if self.order_col else len(df)
        )
        kpis["total_customers"] = (
            df[self.customer_col].nunique()
        ...
        cards = {
            ...
            "hr": [
                ...
                },
                {
                    ...
                    "value": (
                        f"{kpis['avg_efficiency']:.1f}"
                        if kpis["avg_efficiency"] > 0
                        else "N/A"
                    ),
                    "color": "amber",
                    "trend": "Rating"
    ...
    def _daily_orders(self, df):
        ...
        d["day"] = pd.to_datetime(
            d[self.date_col]).dt.strftime("%Y-%m-%d")
        g = d.groupby("day").size().reset_index(
            name="orders")
        return g.sort_values("day").tail(30).to_dict(
            "records")
    ...
    def _category_breakdown(self, df):
        if not self.category_col or not self.revenue_col:
            return []
        d = df[[self.category_col,
                self.revenue_col]].copy()
    ...
    def detect_visualizations(self):
        ...
        return {
            ...
            "available": [
                ...
                {"id":"bar_h",     "type":"bar_h",
                 "title":"Bar chart (H)",       "icon":"📊"},
                {"id":"bar_v",     "type":"bar_v",
                 "title":"Bar chart (V)",       "icon":"📶"},
                {"id":"doughnut",  "type":"doughnut",
                ...
                {"id":"waterfall", "type":"waterfall",
                 "title":"Waterfall chart",     "icon":"🌊"},
                {"id":"combo",     "type":"combo",
                 "title":"Combo (Line + Bar)",  "icon":"🔀"},
            ],
            "department":  dept,
            "dept_config": config,
            "all_depts":   detection["all_depts"],
